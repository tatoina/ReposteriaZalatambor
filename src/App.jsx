...
function Admin({ products, setProducts, orders }) {
  ...
  return <>
    <main className="admin">
      ...
      {tab === 'ingresos' && <section className="admin-panel"><div className="panel-title"><h2>Resumen de ingresos</h2><span>Pedidos cobrados y entregados</span></div><div className="metrics"><Metric icon={<CreditCard />} label="Ingresos cobrados" value={eur.format(revenue)} note="Este mes" /><Metric icon={<ClipboardList />} label="Pedidos activos" value={orders.filter((order) => order.status === 'Pendiente').length} note="Pendientes de preparar" /></div></section>}
