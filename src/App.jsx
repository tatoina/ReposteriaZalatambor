...
export default function App() {
  ...
  const placeOrder = async (event) => {
    ...
    const order = { customer: `${form.get('name')} ${form.get('surname')}`, name: form.get('name'), surname: form.get('surname'), phone: form.get('phone'), address: form.get('address'), delivery: selectedDelivery, items: cart, total: cartTotal + (selectedDelivery ? 5 : 0), paymentMethod: 'Bizum', status: 'Pendiente', date: 'Ahora' };
    try {
  ...
  return <div className="app-shell">
    ...
    {checkout && <Checkout total={cartTotal} delivery={delivery} placeOrder={placeOrder} close={() => setCheckout(false)} />}
