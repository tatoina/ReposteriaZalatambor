...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribeOrders = subscribeOrders(setOrders, reportError);
  ...
  const placeOrder = async (event) => {
    ...
    const order = { customer: `${form.get('name')} ${form.get('surname')}`, name: form.get('name'), surname: form.get('surname'), phone: form.get('phone'), address: form.get('address'), delivery, items: cart, total: cartTotal + (delivery ? 5 : 0), paymentMethod: 'Bizum', status: 'Pendiente', date: 'Ahora' };
    try {
      if (firebaseReady) await createOrder(order);
      else setOrders((current) => [{ id: `RZ-${1043 + current.length}`, ...order }, ...current]);
      setCart([]); setCheckout(false); setShowCart(false); setNotice('Pedido recibido. Te confirmaremos todos los detalles muy pronto.');
    } catch {
      setNotice('No se ha podido guardar el pedido. Inténtalo de nuevo.');
    }
  };
  ...
  return <div className="app-shell">
    <header className="topbar">
      <button className="brand" onClick={() => products.length === 0 ? setShowLogin(true) : setView('shop')} aria-label={products.length === 0 ? 'Acceso al obrador' : 'Ir a inicio'}><span className="brand-mark">Z</span><span>REPOSTERIA<br /><b>ZALATAMBOR</b></span></button>
    ...
    </main> : <Admin products={products} setProducts={setProducts} orders={orders} />}
    ...
    {showCart && <Cart cart={cart} total={cartTotal} updateCart={updateCart} toggleGift={toggleGift} close={() => setShowCart(false)} checkout={startCheckout} />}
    {checkout && <Checkout total={cartTotal} placeOrder={placeOrder} close={() => setCheckout(false)} />}
    {showLogin && <AdminLogin login={loginAdmin} error={loginError} close={() => { setShowLogin(false); setLoginError(''); }} />}
    {showCustomerAuth && <CustomerAuth authenticate={authenticateCustomer} error={customerAuthError} close={() => { setShowCustomerAuth(false); setCustomerAuthError(''); }} />}
  </div>;
}

function Cart({ cart, total, updateCart, toggleGift, close, checkout }) { return <aside className="drawer"><div className="drawer-head"><div><p className="eyebrow">TU SELECCIÓN</p><h2>El carrito</h2></div><button className="icon-button" onClick={close}><X /></button></div>{cart.length === 0 ? <div className="empty"><ShoppingBag size={38} /><p>Tu cesta todavía está vacía.</p></div> : <><div className="cart-lines">{cart.map((item, index) => <div className="cart-line" key={`${item.id}-${index}`}><img src={item.image} alt="" /><div><h3>{item.name}</h3><label><input type="checkbox" checked={item.gift} onChange={() => toggleGift(index)} /> Envoltorio regalo (+2,50 EUR)</label><div className="quantity"><button onClick={() => updateCart(index, -1)}><Minus size={14} /></button><span>{item.quantity}</span><button onClick={() => updateCart(index, 1)}><Plus size={14} /></button></div></div><strong>{eur.format(item.price * item.quantity + (item.gift ? 2.5 * item.quantity : 0))}</strong></div>)}</div><div className="cart-bottom"><div><span>Subtotal</span><strong>{eur.format(total)}</strong></div><button className="primary checkout-btn" onClick={checkout}>Continuar pedido <ChevronRight size={18} /></button></div></>}</aside> }

function Checkout({ total, placeOrder, close }) { return <div className="modal-backdrop"><form className="checkout-modal" onSubmit={placeOrder}><div className="drawer-head"><div><p className="eyebrow">UN ÚLTIMO PASO</p><h2>Datos del pedido</h2></div><button className="icon-button" type="button" onClick={close}><X /></button></div><div className="form-grid"><label>Nombre<input name="name" required /></label><label>Apellidos<input name="surname" required /></label><label>Teléfono<input name="phone" type="tel" required /></label><label className="full">Dirección<input name="address" required /></label></div><fieldset><legend>¿Cómo quieres recibirlo?</legend><label className="choice"><input name="delivery" type="radio" value="no" defaultChecked /> Recoger en el obrador <small>Sin coste</small></label><label className="choice"><input name="delivery" type="radio" value="yes" /> Entrega a domicilio <small>+5,00 EUR</small></label></fieldset><section className="payment-method"><p className="eyebrow">PAGO</p><b>Bizum</b><span>Envía {eur.format(total)} al 611 541 302</span></section><button className="primary checkout-btn" type="submit">Enviar pedido · {eur.format(total)}<CreditCard size={18} /></button></form></div> }
...
function CustomerAuth({ authenticate, error, close }) { const [mode, setMode] = useState('register'); return <div className="modal-backdrop"><form className="checkout-modal login-modal" onSubmit={(event) => authenticate(event, mode)}><div className="drawer-head"><div><p className="eyebrow">ANTES DE CONFIRMAR</p><h2>{mode === 'register' ? 'Crea tu cuenta' : 'Entra en tu cuenta'}</h2></div><button className="icon-button" type="button" onClick={close}><X /></button></div><p className="auth-copy">Tu cuenta te permite consultar tus pedidos y hacer compras más rápido.</p><div className="form-grid"><label className="full">Correo electrónico<input name="email" type="email" autoComplete="email" required /></label><label className="full">Contraseña<input name="password" type="password" autoComplete={mode === 'register' ? 'new-password' : 'current-password'} minLength="6" required /></label></div>{error && <p className="form-error">{error}</p>}<button className="primary checkout-btn" type="submit">{mode === 'register' ? 'Crear cuenta y continuar' : 'Entrar y continuar'} <ChevronRight size={18} /></button><button className="text-button" type="button" onClick={() => setMode(mode === 'register' ? 'login' : 'register')}>{mode === 'register' ? 'Ya tengo una cuenta' : 'Quiero crear una cuenta'}</button></form></div> }
...
function Admin({ products, setProducts, orders }) {
  ...
  return <>
    <main className="admin">
      <div className="admin-header"><p className="eyebrow">PANEL DE OBRADOR · ANDER · {firebaseReady ? 'FIREBASE CONECTADO' : 'PENDIENTE DE CONEXIÓN'}</p><button className="primary" onClick={() => setShowProduct(true)}><Plus size={18} /> Nuevo producto</button></div>
      <div className="metrics"><Metric icon={<CreditCard />} label="Ingresos cobrados" value={eur.format(revenue)} note="Este mes" /><Metric icon={<ClipboardList />} label="Pedidos activos" value={orders.filter((order) => order.status === 'Pendiente').length} note="Pendientes de preparar" /></div>
      <div className="admin-tabs"><button className={tab === 'pedidos' ? 'active' : ''} onClick={() => setTab('pedidos')}>Pedidos</button><button className={tab === 'catalogo' ? 'active' : ''} onClick={() => setTab('catalogo')}>Productos</button><button className={tab === 'servicios' ? 'active' : ''} onClick={() => setTab('servicios')}>Tarifas y servicios</button></div>
